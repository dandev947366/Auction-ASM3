...
var servbot_exports = {};
__export(servbot_exports, {
  default: () => servbot_default
});
module.exports = __toCommonJS(servbot_exports);
...
const RELOAD_HTML = `
    ...
    <\/script>
`;
var servbot_default = ({ root = ".", reload = false, fallback = "", ignores = [], credentials, verbose = false }) => {
  root = (0, import_path.resolve)(root);
  if (!(0, import_fs.existsSync)(root) || !(0, import_fs.statSync)(root).isDirectory()) {
  ...
  const clients = [];
  const createServer = credentials ? (listener) => import_https.default.createServer(credentials, listener) : (listener) => import_http.default.createServer(listener);
  const server = createServer((req, res) => {
    const reqPath = decodeURI(req.url);
    const url = new import_url.URL(reqPath, protocol + req.headers.host);
    let { pathname } = url;
    ...
    if (fallback && !routeHasDot || fallback && routeHasDot && (ignores.length && !matchesIgnores)) {
    ...
    const uri = (0, import_path.join)(root, pathname);
    const ext = uri.split(".").pop();
    if (!(0, import_fs.existsSync)(uri)) {
      return errorResponse(res, pathname, 404, verbose);
    }
    (0, import_fs.readFile)(uri, "binary", (err, file) => {
      return err ? errorResponse(res, pathname, 500, verbose) : fileResponse(res, pathname, 200, file, ext, htmlToAppend, verbose);
    });
  });
  process.on("SIGINT", () => {
...
function routeResponse(res, pathname, root, fallback, htmlToAppend, verbose) {
  const fallbackPath = (0, import_path.join)(root, fallback);
  (0, import_fs.readFile)(fallbackPath, "binary", (err, file) => {
    if (err)
      return errorResponse(res, pathname, 500, verbose);
...
function fileResponse(res, pathname, status, file, ext, htmlToAppend, verbose) {
  ...
  if (GZIP_EXTS.includes(ext)) {
    if (ext === "html" && htmlToAppend)
      file += htmlToAppend;
    res.setHeader("Content-Encoding", "gzip");
    file = (0, import_zlib.gzipSync)(Buffer.from(file, "binary").toString("utf8"));
    encoding = "utf8";
  }
